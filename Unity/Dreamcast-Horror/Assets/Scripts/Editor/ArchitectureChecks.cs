...
public static class ArchitectureChecks
{
    ...
    private static extern void unity_game_step(float x, float z, float seconds);
    [DllImport("dreamcast_horror", CallingConvention = CallingConvention.Cdecl)]
    ...
    public static void Run()
    {
        ...
        {
            EditorSceneManager.OpenScene("Assets/Scenes/SampleScene.unity");
            Check(Object.FindObjectsByType<NativeGameBridge>(FindObjectsSortMode.None).Length == 1,
            ...
            var debug = Object.FindFirstObjectByType<CollisionDebug>();
            ...
            Check(CppCollision.ActiveShapes.Count == 10, "Collision debug registry contains stale entries.");
            ...
            Check(NativeRoomDiagnostics.Validate(text.Replace("shapes 10", "shapes 129")) != null,
                "Native room loader accepted overflow.");

            // Export must refuse overflow before touching the existing file.
            var extras = new GameObject[119];
            ...
            var bridge = Object.FindFirstObjectByType<NativeGameBridge>();
            Call(bridge, "Start");
            ...
            Check(NativeGameBridge.LoadedShapes.Length == 10, "Debug snapshot has the wrong collision count.");
            Call(bridge, "OnDisable");
            Check(!NativeGameBridge.IsInitialized, "Disabled host remained active.");
            Call(bridge, "OnEnable");
            Check(NativeGameBridge.IsInitialized && NativeGameBridge.LoadedShapes.Length == 10,
                "Re-enabled host did not resume the room.");
            unity_game_step(0, 1, 0.8f);
